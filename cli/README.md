...
**Current Status**: ✅ CLI v5.1.6 (simplified surface)
...
Legacy aliases (hidden from `--help` in v5.1.6):
...
CLI v5.1.6 intentionally keeps a small visible surface (`agent setup/check/rca/update/version`). For backwards compatibility and advanced workflows, these commands still exist but are hidden from `agent --help`:
...
│   ├── setup.go         # Interactive setup wizard (v5.1.6)
│   ├── check.go         # Standard diagnostics report (v5.1.6)
│   ├── rca.go           # Post-call RCA (v5.1.6)
